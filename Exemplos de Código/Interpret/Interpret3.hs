...
evalExpr :: ValEnv -> FunEnv -> Expr -> State Store Value
...
free :: Loc -> State Store ()
free loc = do
  store <- get
  put (Map.delete loc store)

update :: Loc -> Value -> State Store ()
update loc val = do
  store <- get
  put (Map.insert loc val store)
...
evalProg (Prog defs main) =
  ...
  in evalState (evalExpr Map.empty ftabl main) Map.empty
